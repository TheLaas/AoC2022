...
StopWatch::StopWatch() : mStart(clock::now()) {
  ...
  // FYI:  This would fail
  // static_assert(std::chrono::high_resolution_clock::is_steady(), "High
  // Resolution Clock is NOT steady on CentOS?!");
}
...
uint64_t StopWatch::ElapsedUs() const {
  return std::chrono::duration_cast<microseconds>(clock::now() - mStart)
      .count();
}

/// @return the elapsed milliseconds since start
uint64_t StopWatch::ElapsedMs() const {
  return std::chrono::duration_cast<milliseconds>(clock::now() - mStart)
      .count();
}
